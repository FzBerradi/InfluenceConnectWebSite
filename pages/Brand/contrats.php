<?php
session_start(); // Démarrer la session pour récupérer les variables de session
if($_SESSION['is_brand']!= true){ // Vérifier si la variable de session "is_brand" est définie à "true"
  header('location:../sign-in.php'); // Rediriger vers la page de connexion si la variable de session n'est pas définie à "true"
}
include('../dbconnect.php'); // Inclure le fichier de connexion à la base de données

// Récupérer les informations des influenceurs associés à la marque qui sont confirmés
$listinfluencerconfirmed=$data->prepare('
    SELECT u.*, p.*
    FROM user u
...
$listinfluencerconfirmed->execute([
    'BrandID'=>$_SESSION['iduser']
]);
$influencerconfirmed=$listinfluencerconfirmed->fetchAll(PDO::FETCH_ASSOC); // Stocker les résultats dans un tableau associatif

// Récupérer les informations des influenceurs associés à la marque qui ne sont pas confirmés
$listinfluencernotconfirmed=$data->prepare('
    SELECT u.*, p.*
    FROM user u
    JOIN partnership p ON u.ID = p.InfluencerID
    WHERE p.BrandID = :BrandID
...
$listinfluencernotconfirmed->execute([
    'BrandID'=>$_SESSION['iduser']
]);
$influencernotconfirmed=$listinfluencernotconfirmed->fetchAll(PDO::FETCH_ASSOC); // Stocker les résultats dans un tableau associatif
?>
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
      ...
      <ul class="navbar-nav">
        ...
        <a class="nav-link  " href="../../pages/messages/message.php">
          <div >
            ...
            <span class="nav-link-text ms-1">Messsages</span>
          </a>
        </li>
        <li class="nav-item">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">contrats</li>
          </ol>
        </nav>
        ...
        <div class="col-12 mt-4">
          <div class="card mb-4">
            <div class="card-header pb-0 p-3">
              <h6 class="mb-1">Les Contrats</h6>
              <p class="text-sm">La liste des contrats confirmés:</p>
            </div>
  ...
  <?php
    foreach ($influencerconfirmed as $brand) {
      echo '            
  ...
  ?>
    ...
    <div class="card mb-4">
            <div class="card-header pb-0 p-3">
              <h6 class="mb-1">Les Contrats</h6>
              <p class="text-sm">La liste des contrats non confirmés:</p>
            </div>
    <div class="container-fluid py-4 cards">
  ...
  <?php
    foreach ($influencernotconfirmed as $brand) {
      echo '            
