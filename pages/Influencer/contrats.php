...
// Récupérer la liste des marques associées à l'influenceur avec un statut "confirmé"
$listbrandsconfirmed=$data->prepare('
  SELECT u.*, p.*
  FROM user u
...
$listbrandsconfirmed->execute([
    'InfluencerID'=>$_SESSION['iduser'] // Récupérer l'ID de l'utilisateur de la session
]);
$brandesconfirmed=$listbrandsconfirmed->fetchAll(PDO::FETCH_ASSOC); // Récupérer les données des marques sous forme de tableau

// Récupérer la liste des marques associées à l'influenceur avec un statut différent de "confirmé"
$listbrandsnotconfirmed=$data->prepare('
  SELECT u.*, p.*
  FROM user u
  JOIN partnership p ON u.ID = p.BrandID
  WHERE p.InfluencerID = :InfluencerID
...
$listbrandsnotconfirmed->execute([
    'InfluencerID'=>$_SESSION['iduser'] // Récupérer l'ID de l'utilisateur de la session
]);
$brandesnotconfirmed=$listbrandsnotconfirmed->fetchAll(PDO::FETCH_ASSOC); // Récupérer les données des marques sous forme de tableau
?>
...
<aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3 " id="sidenav-main">
  ...
  <?php
    foreach ($brandesconfirmed as $brand) {
      echo '            
  ...
  <?php
    foreach ($brandesnotconfirmed as $brand) {
      echo '            
