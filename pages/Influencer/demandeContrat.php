...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si aucune erreur n'est présente, traiter les données du formulaire
    if (empty($errors)) {
        try {
            $BrandID=$_GET['id'];
            $InfluencerID=$_SESSION['iduser'];
            $userdata = $data->prepare("INSERT INTO partnership(BrandID, InfluencerID,InfluencerConfirmed, price, StartDate, EndDate,description,Status) VALUES (:BrandID, :InfluencerID,:InfluencerConfirmed,:price,:StartDate,:EndDate,:description,:Status)");
  
            $userdata->execute([
              'BrandID' => $BrandID,
              'InfluencerID' => $InfluencerID,
              'InfluencerConfirmed' => 1,
              'price' => $price,
